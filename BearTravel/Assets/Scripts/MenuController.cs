...
public class MenuController : MonoBehaviour {
    ...
    {
        float y = GvrControllerInput.TouchPos.x - 0.5f; // Get the y position after subtracting the center point
        ...
        switch (_currentState)
        {
            case MenuScrollState.None:
                ...
                { // If we're currently in the None state and we're holding far enough up // on the touchpad, we transition to the down state SetState(MenuScrollState.Up, TimeDelay); } break; case MenuScrollState.Down: if (y > DirectionSensitivity)
            ...
            case MenuScrollState.Up:
                if (y < -DirectionSensitivity)
                { // If we're in the up state and we're still still holding far enough up // on the touchpad, we should go to the previous button in our menu. ShouldSelectNextButton(-1); } else if (y > DirectionSensitivity)
